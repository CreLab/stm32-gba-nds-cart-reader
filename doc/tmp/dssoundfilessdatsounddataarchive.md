...
|       00Eh 2                                                          |
|      Number of Blocks (usually 4 = SYMB+INFO+FAT+FILE) (or 3=no SYMB) |
|       010h 4+                                                         |
| 4   SYMB Block (Offset from SDAT+0, Size) ;=0,0 if above is 3=no SYMB |
...
The SYMB block exists in most SDAT files (except in some titles like
Downhill Jam and Over the Hedge).


```
+-----------------------------------------------------------------------+
|      _________                                                        |
| ________________________ SYMB Block _________________________________ |
+-----------------------------------------------------------------------+
```


**Symbol Block (if present) (names for the corresponding items in INFO
block)**
...
|       000h 4     ID "SYMB"                                            |
|       004h 4                                                          |
|    SYMB Block Size (rounded up to 4-byte boundary, unlike as in SDAT) |
|                                                                       |
|   008h 4     File List SSEQ    (Offset from SYMB+0) Sequences (songs) |
|       00C                                                             |
| h 4     Folder List SSAR  (Offset from SYMB+0) Sequence Archives (fx) |
|       010h 4     File List BANK    (Offset from SYMB+0) Banks         |
|       014h                                                            |
|  4     File List SWAR    (Offset from SYMB+0) Wave Archives (samples) |
|       018                                                             |
| h 4     File List Player  (Offset from SYMB+0) Player (Group-related) |
|       01Ch 4                                                          |
|    File List Group   (Offset from SYMB+0) Group (SSEQ+SSAR+BANK+SWAR) |
|       020h                                                            |
| 4     File List Player2 (Offset from SYMB+0) Player2 (Stream-related) |
...
|       000h 4       Number of entries in this list (can be 0=None)     |
|       004h N                                                          |
| *(4+4) SSAR "Folder Name" and SSEQ "File List" (Offset's from SYMB+0) |
+-----------------------------------------------------------------------+
```
...
|       00C                                                             |
| h 4     Info List SSAR    (Offset from INFO+0) Sequence Archives (fx) |
|       010h 4     Info List BANK    (Offset from INFO+0) Banks         |
|       014h                                                            |
|  4     Info List SWAR    (Offset from INFO+0) Wave Archives (samples) |
|       018                                                             |
| h 4     Info List Player  (Offset from INFO+0) Player (Group-related) |
|       01Ch 4                                                          |
|    Info List Group   (Offset from INFO+0) Group (SSEQ+SSAR+BANK+SWAR) |
|       020h                                                            |
| 4     Info List Player2 (Offset from INFO+0) Player2 (Stream-related) |
...
**SSAR Info Entry (Sequence Archives, eg. collections of sound
effects)**
...
Note: bnk/vol/cpr/ppr/ply is stored in the SSAR file (instead of in
Info). The actual sequences are also in SSAR? Or does the SSAR contain
pointers to SSEQ files?
...
**SWAR Info Entry (Wave Archives, eg. collections of instruments or
effects)**
...
**Group Info Entry (whatever, list of files that must be loaded to
memory?)**
...
|       001h                                                            |
- |  16  v[16]              ;unknown array (UNUSED entries are set to FFh |
|       011h 7   Reserved (0)                                           |
+-----------------------------------------------------------------------+
```
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| ___________________ FAT and FILE Blocks _____________________________ |
+-----------------------------------------------------------------------+
```
...
|       00Ch 4         Reserved (0)                                     |
|       010h ..                                                         |
|        Files (SSEQ,SSAR,SBNK,SWAR,STRM) (at offsets specified in FAT) |
+-----------------------------------------------------------------------+
```
