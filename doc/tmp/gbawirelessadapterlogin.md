...
|      @@lop:                                                           |
|       passes=                                                         |
| passes+1, if passes>32 then ERROR  ;give up (usually only 10 passses) |
|       recv.lo=siodata AND FFFFh    ;response from adapter             |
|       recv.hi=siodata/10000h       ;adapter's own "NI" data           |
|       if                                                              |
| send.hi<>recv.lo then index=0, goto @@stuck  ;<-- fallback to index=0 |
...
|       ret                                                             |
|      @@ke                                                             |
| y_string db 'NINTENDO',01h,80h    ;10 bytes (5 halfwords; index=0..4) |
+-----------------------------------------------------------------------+
```
