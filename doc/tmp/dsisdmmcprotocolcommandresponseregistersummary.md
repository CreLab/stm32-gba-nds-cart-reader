...
|       CMD0                                                            |
|      mmc        GO_PRE_IDLE_STATE (CMD0 with arg=F0F0F0F0h) (type=bc) |
|       CM                                                              |
| D0     mmc        BOOT_INITIATION (CMD0 with arg=FFFFFFFAh, type=N/A) |
...
|       CMD5                                                            |
|     sd     spi Reserved for I/O cards (see "SDIO Card Specification") |
|       CMD5                                                            |
|  mmc    ?   SLEEP_AWAKE (type=ac) (MMC only, IO_SEND_OP_COND on SDIO) |
|       CMD                                                             |
...
|       CMD1                                                            |
| 3    sd/mmc spi GET_STATUS (type=ac) (sends 16bit status in SPI Mode) |
|                                                                       |
|  CMD14    mmc        BUSTEST_R (type=adtc) (MMC only, Reserved on SD) |
|       CMD19                                                           |
|   mmc        BUSTEST_W (type=adtc) (MMC only, SET_TUNING_BLOCK on SD) |
...
|       CMD22    sd         Reserved                                    |
|       CMD23                                                           |
|     sd/mmc-spi SET_BLOCK_COUNT (type=ac) (SPI supported ONLY on MMC?) |
+-----------------------------------------------------------------------+
```
...
|       CMD3                                                            |
| 2-34 mmc    spi Reserved for compatibility with older MMC cards (uh?) |
|       CMD35    mmc    spi ERASE_GROUP_START (type=ac)                 |
|       CMD36    mmc    spi ERASE_GROUP_END (type=ac)                   |
|       CMD3                                                            |
| 7    mmc    spi Reserved for compatibility with older MMC cards (uh?) |
...
|       CMD16    sd/mmc spi SET_BLOCKLEN (type=ac)                      |
|       CMD40                                                           |
|   sd         Defined by DPS Spec (Data Protection System) (type=adtc) |
...
+-----------------------------------------------------------------------+
|       CMD                                                             |
| 6     mmc    spi SWITCH      (type=ac)   ;related to EXT_CSD register |
...
+-----------------------------------------------------------------------+
|       CMD21                                                           |
|    sd         Reserved for DPS Specification (Data Protection System) |
...
**Below CMD58-59 SPI-only (in Non-SPI mode: MMC=Reserved,
SD=EXTR_MULTI)**
...
Above two commands are supported in SPI mode only, and are supported for
both SD and MMC (though newer MMC docs are no longer mentioning them
since JEDEC dropped SPI support).
...
|       ACMD7-12  -         Reserved                                    |
|       ACMD14                                                          |
| -16 sd        Reserved for DPS Specification (Data Protection System) |
|       ACMD17    -         Reserved                                    |
...
|       RCA                                                             |
|        16bit  sd/mmc     Relative Card Address (not used in SPI mode) |
|       DSR       16bit  sd/mmc spi Driver Stage Register (optional)    |
|       SSR                                                             |
|     512bit  sd     spi SD Card Status Register: Extended status field |
|       SCR       64bit  sd     spi SD Card Configuration Register      |
|       EX                                                              |
| T_CSD 4096bit  mmc    spi MMC Extended CSD Register (status & config) |
|       PWD      128bit  sd/mmc spi Password (Card Lock) (max 16 bytes) |
|       PW                                                              |
| D_LEN    8bit  sd/mmc spi Password Length (0..16 max) (0=no password) |
+-----------------------------------------------------------------------+
```
...
|       R                                                               |
| 1b   48bit  Busy CMDs/ACMDs     32bit CSR Card Status (and DATA=busy) |
...
|       R6        -  -                   Reserved                       |
|       R7    4                                                         |
| 0bit  CMD8                8bit CSR and 32bit Card interface condition |
|       ERROR                                                           |
|  8bit  Only first 8bit sent upon Illegal Command or Command CRC Error |
+-----------------------------------------------------------------------+
```


**Commands with Data Transfers (additionally to command/response)
(type=adtc)**
...
|       CMD9                                                            |
|     R   sd/mmc spi GET_CSD (128bit)  ;\in SPI Mode only (Non-SPI mode 
|       CMD10                                                           |
|   R   sd/mmc spi GET_CID (128bit)  ;/sends that info as CMD response) |
...
|       CMD3                                                            |
| 1    R   mmc        GET_WRITE_PROT_TYPE (32x2bit write-protect types) |
...
**Misnamed Commands**
Official command names include various SEND_xxx commands, which are
misleading because they don\'t indicate if they \"send\" informatio
\<to\> or \<from\> the card (or both). Better naming would be GET_xxx
SET_xxx, or GET_SET_xxx.
...
SD-TRAN driver of host should manage the difference of SD commands
functions. Not supported commands should not issue to UHS-II card. CMD13
shall not be issued during data transfer. Normally, data transfer should
be stopped by setting TLEN instead of using CMD12.
...
|      CMD13  De                                                        |
| vice operation is up to implementation during data transfer (eg. CTS) |
|      CMD11  Illegal                                                   |
|      CMD1                                                             |
| 2  Normally, TLEN (data length) in UHS-II packet is used to stop data |
|             transfer.                                                 |
|      CMD12                                                            |
|   Should be used to abort an operation when illegal situation occurs. |
|      CMD15  Illegal                                                   |
|      CMD19  Illegal                                                   |
|      CMD23                                                            |
...
Not Affected means that the command is not executed in any card state,
and response is returned (response type is up to implementation).
Illegal means that card returns response with NACK=1.
As SDHC/SDXC Cards do not support CMD28, 29 and 30, these commands are
also illegal in UHS-II mode.

**Note**
All future reserved commands shall have a codeword length of 48 bits, as
well as their responses (if there are any).
