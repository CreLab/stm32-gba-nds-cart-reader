...
|       SFR_CS                                                          |
|   ... can be manually changed BEFORE JMP/CALL AX to make FAR-JMP... ? |
+-----------------------------------------------------------------------+
```
...
|      @@def sfr                                                        |
| p             sfrp       16-bit-manipulatable SFR name (even address) |
|                                                                       |
...
|      @@def X                                                          |
| Xaddr20_code    !!addr20   20bit jumpdest (3 bytes) (CALL/BR) to CS=x |
|      @@def Xa                                                         |
| ddr16_code     !addr16    16bit jumpdest (2 bytes) (CALL/BR) to CS=0! |
|      @@def X                                                          |
| addr16          !addr16    16bit data addr                    at ES=F |
- |      @@def IhlI             [HL]           ;[HL]        ;\            
|      @@def I                                                          |
- | deI             [DE]           ;[DE]        ; maybe can also use ES:? |
...
|      @@def Is                                                         |
- | p_byteI        [SP+byte]      ;[SP+nn]     ;\shouldn't/can't use ES:? 
- |      @@def I00nnI           [00nn]         ;[000nn]     ;/            |
+-----------------------------------------------------------------------+
```

Caution: 16bit MOVW/ADDW/etc with memory operand must use even addresses
- (this is documented only for \[sfrp\] and \[saddrp\], but does als
- apply for \[nnnn\], which does actually read/write memory at \[nnnn an
FFFEh\]).
...
|       ?  w                                                            |
- | ord[B], word[C] (only the space from F0000H to FFFFFH is specifiable) |
|                                                                       |
- |     ?  word[BC] (only the space from F0000H to FFFFFH is specifiable) |
|       ?  ES                                                           |
- | :[HL + byte], ES:[DE + byte] (higher 4-bit addresses are specified by |
|           the ES register)                                            |
|                                                                       |
...
+-----------------------------------------------------------------------+
|       00001                                                           |
| F02 MCU    v0, v1026, v2048, v3072, v4102, v5122, v6145, v7168, v8192 |
...
**RL78/G13 - Memory Map (R5F100xA, R5F101xA (x = 6 to 8, A to C, E to
G))**
