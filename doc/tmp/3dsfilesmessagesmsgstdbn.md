...
Message files contain text messages, usually there are separate message
files for different languages (in separate folders).
...
|       00Eh 2                                                          |
|     Number of following Chunks (4=LBL1+ATR1+TXT2+TSY1 or 2=NLI1+TXT2) |
|       010h 2     Zero (0)                                             |
...
|       008h 8       Padding (0)                                        |
|       010                                                             |
| h 4       Number of Hash Table Entries (always 65h in MsgStdBn files) |
|                                                                       |
...
|       004h 4                                                          |
|        Offset to 1st matching label (from LBL1+10h) (garbage if None) |
+-----------------------------------------------------------------------+
```
...
|       0                                                               |
| 06h 2     Number of following parameter bytes (0=None) (usually even) |
|       008h N     Parameters                                           |
+-----------------------------------------------------------------------+
```
...
There is probably also something for inserting User Name or other
strings and numeric variables, sound effects, and perhaps something for
...
|       004h 4                                                          |
|      Chunk Size (minus 10h, and excluding 16-byte padding) (NumMsg*4) |
|       008h 8     Zero (0)                                             |
|                                                                       |
...
Uh, 3DS System Settings has Style Index values 0, 1, 2, and FFFFFFFFh
(but doesn\'t has a .msbt file at all).

**NLI1 Chunk (reportedly used in some files, instead of
LBL1/ATR1/TSY1?)**
...
Hash Tables
Many items (such as messages in msbt files or colors in msbp files) are
looked up by label. The labels are looked up with a hash table and are
stored in a different block than the items themselves. In official files
the hash table always has a fixed number of slots (101 in msbt files, 29
in msbp files), even if it contains only a few labels.
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| _________________ MsgPrjBn (aka .msbp file)__________________________ |
+-----------------------------------------------------------------------+
```
...
|       00Ah 2     Zero (0)                                             |
|       00C                                                             |
| h 1     Message encoding (0=UTF-8, 1=UTF-16, 2=UTF-32) (usually 0 ??) |
|       00Dh 1     Version number (always 3)                            |
|                                                                       |
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| _____________________ MsgPrjBn Colors _______________________________ |
+-----------------------------------------------------------------------+
```
...
|       010                                                             |
| h 4       Number of Hash Table Entries (always 1Dh in MsgPrjBn files) |
|                                                                       |
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| ___________________ MsgPrjBn Attributes _____________________________ |
+-----------------------------------------------------------------------+
```
...
The Hash slots can be all empty without any strings (eg. as so in
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| ______________________ MsgPrjBn Tags ________________________________ |
+-----------------------------------------------------------------------+
```
...
Tag Groups can be \"UserName\", \"DayTime\", \"ErrorInfo\", fo
example.
...
Tags can be \"MyName\", \"SenderName\" \"Hour\", \"Minute\"
...
Parameter names can be \"face\", \"percent\", \"r\", \"g\", \"b\"
\"a\", \"name\", for example.
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| _____________________ MsgPrjBn Styles _______________________________ |
+-----------------------------------------------------------------------+
```
...
|       00                                                              |
| 8h 4       Font index (eg. 0,1,2 for type, or small/med/large maybe?) |
|                                                                       |
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| _____________________ MsgPrjBn Content ______________________________ |
+-----------------------------------------------------------------------+
```


**CTI1 Chunk (Project Content Info)**
This block contains the names of the source files that the MSBT files
were generated from. The source files have the .mstxt extension.
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| _______________________ RI.mstl file ________________________________ |
+-----------------------------------------------------------------------+
```
