...
\"backdoor\", this will allow yourself (and other people) to uploa
programs much faster as when using the normal GBA BIOS multiboot
function. Aside from the improved transmission speed, there\'s no nee
to reset the GBA each time (eventually manually if you do not have reset
connect), without having to press Start+Select (if cartridge inserted),
and, most important, the time-consuming nintendo-logo intro is
bypassed.
...
actual boot procedure, and pass control to that procedure.

+-----------------------------------------------------------------------+
```
|       Send (PC)    Reply (GBA)                                        |
|       "BR                                                             |
| ST"       "BOOT"        ;request burst, and reply <prepared> for boot |
|       <wa                                                             |
| it 1/16s> <process IRQ> ;long delay, allow slave to enter IRQ handler |
|       ll                                                              |
| llllll     "OKAY"        ;send length in bytes, reply <ready> to boot |
|       d                                                               |
| ddddddd     --------      ;send data in 32bit units, reply don't care |
|       ccc                                                             |
| ccccc     cccccccc      ;exchange crc (all data units added together) |
```
+-----------------------------------------------------------------------+
...
no\$gba built-in assembler, see no\$gba utility menu). When usin
other/mainstream assemblers, you\'ll eventually have to change som
directives, convert numbers from NNNh into 0xNNN format, and define the
origin somewhere in linker/makefile instead of in source code.
...
|      org 2                                                            |
| 000000h    ;origin in RAM for multiboot-cable/no$gba-cutdown programs |
|      ;------------------                                              |
...
|       dcb                                                             |
|  0,0,0,0,  0,0            ;-game code (4 bytes), maker code (2 bytes) |
|       dcb   96                                                        |
| h,0,0                  ;-fixed value 96h, main unit code, device type |
...
|       dcb   0,                                                        |
| 0                      ;-multiboot reserved bytes (destroyed by BIOS) |
...
- |       strh   r0,[r3,2]               ;/                               |
|       ldr                                                             |
- | h   r3,[r1,-8]              ;\mix up with BIOS irq flags at 3007FF8h, 
|       orr                                                             |
|  r3,r3,r0                ; aka mirrored at 3FFFFF8h, this is required |
|       strh                                                            |
- |  r3,[r1,-8]              ;/when using the (VBlank-)IntrWait functions |
|       and    r3,r0,80h ;IE/IF.7 SIO  ;\                               
|                                                                       |
| cmp    r3,80h                  ; check if it's a burst boot interrupt |
|       ldre                                                            |
- | q  r2,[r1,120h] ;SIODATA32 ; (if interrupt caused by serial transfer, |
|                                                                       |
...
- |       ldr   r4,[msg_okay]            ;\                               
|       bl    si                                                        |
| o_transfer             ; receive transfer length/bytes & reply "OKAY" |
|       mov   r2,r0 ;len               ;/                               |
|       mov   r3,3000000h   ;dst       ;\                               
|       mov   r4,0  ;crc               ;                                |
|      @@lop:                          ;                                |
|       bl                                                              |
|   sio_transfer             ; download burst loader to 3000000h and up |
- |       stmia [r3]!,r0      ;dst       ;                                |
|       add   r4,r4,r0      ;crc       ;                                |
|       subs  r2,r2,4       ;len       ;                                |
|       bhi   @@lop                    ;/                               |
...
|       b                                                               |
|  3000000h  ;ARM state!    ;-launch actual transfer / start the loader |
|      ;------------------                                              |
|      sio_tran                                                         |
| sfer:  ;serial transfer subroutine, 32bit normal mode, external clock |
...
|       bne   @@lop                    ;/                               |
|       sub   r1                                                        |
| 5,lr,8000000h-2000000h ;-return (retadr rom/8000XXXh -> ram/2000XXXh) |
|      ;------------------                                              |
|      init_interrupts:                                                 |
|                                                                       |
...
|       mov  r0,0                      ;\                               
|       s                                                               |
- | tr  r0,[r4,134h] ;RCNT        ; init SIO normal mode, external clock, |
|                                                                       |
|  ldr  r0,=5080h                 ; 32bit, IRQ enable, transfer started |
|       str  r0,                                                        |
- | [r4,128h] ;SIOCNT      ; output "BOOT" (indicate burst boot prepared) |
- |       ldr  r0,[msg_boot]             ;                                |
...
|       bl   d                                                          |
| ownload_rom_to_ram       ;-download ROM to RAM (returns to ram_start) |
|      ram_start:   ;entry point for multiboot/burstboot                |
|       mov                                                             |
| r0,0feh                   ;\reset all registers, and clear all memory 
|       swi                                                             |
| 10000h ;RegisterRamReset  ;/(except program code in wram at 2000000h) |
...
|      @@mainloop:                                                      |
|       sw                                                              |
| i  50000h ;VBlankIntrWait    ;-wait one frame (cpu in low power mode) |
|                                                                       |
