...
+-----------------------------------------------------------------------+
|      _________                                                        |
| ________________ Triangle Drawing Examples __________________________ |
+-----------------------------------------------------------------------+
```


**gpu_draw_triangle_direct: ;simple, but requires a messy 4x24bit data
format**
...
- |       [GPU                                                            |
| REG_FIXEDATTRIB_INDEX]=0Fh  ;mode for directly writing vertex data... |
|       for i=0 to (4*2)-1                                              |
- |         [GP                                                           |
- | UREG_FIXEDATTRIB_DATA+0]=[@@array+i*0Ch+8] ;\write 4x24bit as 3x32bit 
- |         [GPU                                                          |
- | REG_FIXEDATTRIB_DATA+0]=[@@array+i*0Ch+4] ; upper 32bit written first |
...
|       .float2                                                         |
| 4 -1.0  ,-1.0  ,0.0   ,1.0   ,1.0   ,0.5   ,0.5   ,1.0 ;\1st triangle 
|       .                                                               |
| float24 1.0   ,0.4   ,0.0   ,1.0   ,0.5   ,1.0   ,0.5   ,0.5 ;  ;\2nd 
|       .float                                                          |
| 24 0.2   ,1.0   ,0.0   ,1.0   ,0.5   ,0.5   ,1.0   ,0.5 ;/ ; triangle |
|       .float24                                                        |
|  1.0   ,-0.8  ,-1.0  ,1.0   ,0.5   ,0.5   ,1.0   ,0.5    ;/(of strip) |
|       ;note: coord.W must be 1.0 (to avoid perspective division)      |
+-----------------------------------------------------------------------+
```


**gpu_draw_triangle_via_buffer: ;example with 1-2 arrays, optional
index_list**
...
- |       [GPUR                                                           |
| EG_ATTR_BUF_FORMAT_LOW]=9Ch  ;Signed8bit(x,y,z,w)+Unsigned8bit(r,g,b) |
|                                                                       |
...
- |        [GPUREG                                                        |
| _ATTR_BUFi_CONFIG_HIGH+0*0Ch]=(@@numattr shl 28)+((4+3) shl 16)+BA98h |
|                                                                       |
...
- |      ;[GPU                                                            |
| REG_START_DRAW_FUNC0]=00h              ;not needed, 0=draw, 1=config? |
- |       [GPU                                                            |
| REG_ATTR_BUF_NUMVERTICES]=4*(1+@@use_index_list)  ;needed, array size |
...
- |         [G                                                            |
| PUREG_ATTR_BUF_DRAWARRAYS]=0     ;start drawing at above index number |
|       endif                                                           |
|       gpu_copy_colorbuf_to_lcd_framebuf                               |
|       ret                                                             |
|      align 10h                                                        |
...
|     @@array: ;x______y______z______w______r______g______b______a_____ |
|        db                                                             |
|   -7fh  ,-7fh  ,0     ,+7fh  ,0ffh  ,0     ,0          ;\1st triangle 
|        d                                                              |
| b       +7fh  ,+40h  ,0     ,+7fh  ,0     ,0ffh  ,0          ;  ;\2nd 
|        db                                                             |
|    +20h  ,+7fh  ,0     ,+7fh  ,0     ,0     ,0ffh       ;/ ; triangle |
|        db                                                             |
|    +7fh  ,-60h  ,0     ,+7fh  ,0     ,0ffh  ,0ffh          ;/of strip |
|      elseif @@num_arrays=2                                            |
...
+-----------------------------------------------------------------------+
|      _________                                                        |
| ____________ Init and Memory Transfer Functions _____________________ |
+-----------------------------------------------------------------------+
```
...
+-----------------------------------------------------------------------+
|       DummyRe                                                         |
- | ad=[GPUREG_STAT_NUM_TRIANGLES_DISPLAYED]  ;waits until rendering done |
...
- |       [GPUR                                                           |
| EG_VIEWPORT_V_STEP]=38111100h  ;2/240 ;db 00h // .float24 0.008333333 |
- |       [GPUR                                                           |
| EG_VIEWPORT_H_STEP]=37999900h  ;2/320 ;db 00h // .float24 0.006250000 |
...
- |       [GPURE                                                          |
| G_SH_OUTATTR_CLOCK]=3             ;bit0=blah?, bit1=needed for colors |
...
- |       [GPUREG                                                         |
| _RESTART_PRIMITIVE]=0  ;required for strips (even BEFORE first strip) |
...
- |       [GPUREG                                                         |
| _VSH_CODETRANSFER_DATA+0]=4C000000h  ;MOV o0,v0  ;used for coord xyzw |
- |       [GPUREG                                                         |
| _VSH_CODETRANSFER_DATA+0]=4C201000h  ;MOV o1,v1  ;used for color rgba |
