...
- |       TSC[3:02                                                        |
| h]=18h  ;SAR ADC clk divider, Div8 (12bit mode) (RMW bit34=3, bit7=?) |
...
This is needed to unmute the microphone (otherwise microphone will
return only 0000h or FFFFh.
...
**basic_tsc_init_list:**
This is done by DSi Launcher/Unlaunch (and other titles usually won\'
need to do this).
...
|       TSC                                                             |
- | [1:30h]=40h  ;P-Terminal Delta-Sigma Mono ADC Channel Fine-Gain Input |
...
Switches to NDS Touchscreen/sound mode. Usually done when starting NDS
- titles (or DSi titles that request NDS-TSC mode in carthdr\[1BFh\]).
...
- |       TSC[1:21h]=20h  ;HP Output Drivers POP Removal Settings         |
|       TSC                                                             |
- | [1:22h]=F0h  ;Output Driver PGA Ramp-Down Period Control (70h OR 80h) |
|                                                                       |
...
- And, set powerman\[0\].bit0=1 (bit0=sound amplifier on) (this i
actually part of TSC chip, accessed via \"POWERMAN\" SPI chipselec
signal).
