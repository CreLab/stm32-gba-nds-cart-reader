...
- |       [0FFD                                                           |
| Bh].3  Port 8 Data bit3 OUT  Savedata chipselect (0=select) (cmd 00h) |
- |       [0FFD                                                           |
| Bh].2  Port 8 Data bit2 OUT  LED color    ;\used in UNUSED functions, 
|                                                                       |
- | [0FFDBh].3  Port 8 Data bit3 OUT  LED color    ; in OLD ROM only, and |
- |       [0FFD                                                           |
| Eh].0  Port B Data bit0 IN   Button input ;/conflicting with Savedata |
|       IrDA  IR Transfers                                              |
...
|       FD8Ah 1                                                         |
|     initial_blah     ;-initial state of Port 8.bit3 (not really used) |
|                                                                       |
| FD8Bh 1     ir_rxbuf_wrptr   ;-ir_rxbuf_wrptr (for incoming IR data)? |
|       F                                                               |
| D8Ch 1     ir_rxbuf_rdptr   ;-ir_rxbuf_rdptr (for forwarding to spi)? |
|       FD8Dh 84h   spi_rx_buf       ;-spi_rx_buf  ;(also ir TX buf)    |
|       FE11h 84h   infrared_rx_buf  ;-infrared_rx_buf                  |
|       FE95h 1     spi_index        ;-spi_index                        |
|       FE96h                                                           |
|  1     ir_tx_index      ;-ir_tx_index   (from spi buf to TX infrared) |
|       FE                                                              |
| 97h 1     ir_timeout_flag  ;-ir_timeout_flag   (or packet end or so?) |
...
|       FE44h B8                                                        |
| h+1 infrared_rx_buf  ;-infrared_rx_buf (plus space for appending 00h) |
|       FE                                                              |
| FDh 1     ir_timeout_flag  ;-ir_timeout_flag   (or packet end or so?) |
