...
- |         pow[i]=x, log[x]=i                                            |
|       for i                                                           |
| =0 to 0FFh               ;generate the forward and reverse S-boxes... |
- |         x=pow[0FFh-log[i]]                                            |
|                                                                       |
...
- |         FSb[i]=x, RSb[x]=i                                            |
|       for                                                             |
| i=0 to 0FFh               ;generate the forward and reverse tables... |
...
- |       pow[0                                                           |
| 0h..FFh] = 01,03,05,0F,11,..,C7,52,F6,01   ;pow  ;\needed temporarily 
- |       log[0                                                           |
| 0h..FFh] = 00,FF,19,01,32,..,C0,F7,70,07   ;log  ;/for table creation |
...
|      aes_setkey_results:                                              |
|       key = "                                                         |
| AES-Test-Key-Str-1234567-Abcdefg"  ;use only 1st bytes for 128/192bit |
|       128bit                                                          |
- | ENCRYPT --> RK[0..9..30..43] = 2D534541..ED0DC6FA..43DAC81C..0F5026BB |
|       192bit                                                          |
- | ENCRYPT --> RK[0..9..30..51] = 2D534541..4AAB3D82..29CA38D2..CA4DFE3B |
|       256bit                                                          |
- | ENCRYPT --> RK[0..9..30..59] = 2D534541..1AA51359..CCB886C8..88956C9C |
|       128bit                                                          |
- | DECRYPT --> RK[0..9..30..43] = F653079B..47DD8A1C..1C2070A7..7274532D |
|       192bit                                                          |
- | DECRYPT --> RK[0..9..30..51] = 3CEC6AFF..C4F96B6F..AE36B4AE..7274532D |
|       256bit                                                          |
...
|       Une                                                             |
- | ncrypted:   [dta+0..113Fh] = "Unencrypted-Data", 190h x "TestPadding" |
- |       AES-ECB:       [dta+0..113Fh] = 5F,BD,04,DB,..,E4,07,F4,B6  ;\  
- |       AES-CBC:       [dta+0..113Fh] = 0B,BB,53,FA,..,DD,28,6D,AE  ;   |
|       AES-                                                            |
- | CFB128:    [dta+0..113Fh] = F4,75,4F,0E,..,73,B5,D7,E7  ; keysize=128 |
...
|       AES-                                                            |
- | ECB:       [dta+0..113Fh] = 0E,69,F5,1A,..,9A,5F,7A,9A  ;-keysize=192 |
|       AES-                                                            |
- | ECB:       [dta+0..113Fh] = C6,FB,68,C1,..,14,89,6C,E0  ;-keysize=256 |
+-----------------------------------------------------------------------+
```
