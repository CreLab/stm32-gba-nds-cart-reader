...
These events must be done somewhat in sequence. There is some
flexibility as to how they can be ordered but it\'s best to follow thi
order:
...
- |         BB[01                                                         |
- | h]=temp AND 7Fh        ; reset BB[01h].Bit7, then restore old BB[01h] |
|         BB                                                            |
- | [01h]=temp                ; (that BB setting enables the RF9008 chip) |
|       ENDIF                         ;/                                |
|       delay                                                           |
| 30 ms                   ;-(more killer-delay now getting REALLY slow) |
|       call in                                                         |
- | it_sub_functions       ;- same as "Init 16 registers by firmware[..]" |
|                                                                       |
...
|       reg                                                             |
- | [010h] = FFFFh   - W_IF             ;acknowledge/clear any interrupts |
- |       reg[254h] = 0000h   - W_CONFIG_254h    ;                        |
- |       reg[0B4h] = FFFFh   - W_TXBUF_RESET    ;--reset all TXBUF_LOC's |
- |       reg[080h                                                        |
| ] = 0000h   - W_TXBUF_BEACON   ;disable automatic beacon transmission |
...
|       r                                                               |
- | eg[076h] = 0000h   - W_TXBUF_GAPDISP  ;disable gap/skip (offset=zero) |
+-----------------------------------------------------------------------+
```
...
|       for i                                                           |
- | =0 to BYTE firmware[042h]-1 ;number of entries (usually 0Ch) (0..0Bh) |
...
+-----------------------------------------------------------------------+
|       copy 6                                                          |
- | bytes from firmware[036h] to mac address at 0x04800018  (why again ?) |
+-----------------------------------------------------------------------+
```
...
+-----------------------------------------------------------------------+
- |       reg[02                                                          |
| Ch]=0007h  ;W_TX_RETRYLIMIT - XXX needs to be set for every transmit? |
...
\-- and receive data, there are some more variables that need to b
set.
...
- |       reg[1                                                           |
| 34h] = FFFFh -- W_POST_BEACON ;reset post-beacon counter to LONG time |
...
+-----------------------------------------------------------------------+
- |       reg[                                                            |
| 030h] = 8000h    W_RXCNT         ;enable RX system (done again below) |
...
- |       reg[05                                                          |
| Ah] = 0C00h/2  W_RXBUF_READCSR     ;fifo end, same as begin at start. |
- |       reg[06                                                          |
| 2h] = 5F60h-2  W_RXBUF_GAP     ;(set gap<end) (zero should work, too) |
- |       reg[030                                                         |
| h] = 8001h    W_RXCNT  ;enable, and latch new fifo values to hardware |
+-----------------------------------------------------------------------+
```
...
- |       reg[0                                                           |
| D0h] = 0181h    W_RXFILTER set to 0x581 when you successfully connect |
|                                                                       |
...
|       r                                                               |
- | eg[00Ah] = 0000h  -- ? W_X_00Ah      ;(related to rx filter) (again?) |
- |       reg[004h] = 0001h  -- W_MODE_RST      ;hardware mode            |
- |       reg[0                                                           |
| E8h] = 0001h  -- W_US_COUNTCNT   ;enable microsecond counter (again?) |
|                                                                       |
- |     reg[0EAh] = 0001h  -- W_US_COMPARECNT ;enable microsecond compare |
|       reg                                                             |
- | [048h] = 0000h  -- W_POWER_?    ;[disabling a power saving technique] |
- |       reg[038h].Bit1 = 0 -- W_POWER_TX   ;[this too]                  |
|       r                                                               |
- | eg[048h] = 0000h  -- W_POWER_?    ;[umm, it's done again. necessary?] |
- |       reg[0AEh] = 0002h  -- W_TXREQ_SET  ;                            |
- |       reg[03Ch                                                        |
| ].Bit1 = 1 -- W_POWERSTATE ;queue enable power (RX power, we believe) |
- |       reg[0ACh] = FFFFh  -- W_TXREQ_RESET;reset LOC1..3               |
+-----------------------------------------------------------------------+
```

That\'s it, the DS should be now happy to send and receive packets.
It\'s very possible that there are some unnecessary registers set i
here.
