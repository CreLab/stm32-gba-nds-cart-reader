...
DARC archives are used in various places, sometimes uncompressed, or
often with LZSS or LZ11 compression. For example,
...
|       LZ11                                                            |
| compressed DARC  .arc.LZ files (eg. in 3DS Camera RomFS "lyt" folder) |
+-----------------------------------------------------------------------+
```

The DARC format is about same as the Wii\'s U8 format (with differen
header, different byte-order, and UTF-16 instead of ASCII names).
...
|       00Ch                                                            |
| 4    Total Filesize (excluding trailing 20h-byte SHA256-HMAC, if any) |
...
|       Entry1:                                                         |
| Type=Folder, Name=".", Offset=0, End=TotalNumDirEntries ;Dot (if any) |
+-----------------------------------------------------------------------+
```

Entry0/Root is important to know the total directory size, and to
compute the begin of the Name Area:
...
Entry1/Dot is somewhat useless, but it does exist in most (not all) darc
files, ie. the path strings can appear as so:
...
It\'s best to hide the \".\" in path strings (ie. detect if Entry1 is 
Folder, with name \".\" and same Offset/End as Entry0, and if so
skip/ignore it).
...
|      When Type=00h=File:                                              |
|       004h 4                                                          |
|    File offset (relative to begin if darc; NOT to begin of Data Area) |
|       008h 4    File size                                             |
|      When Type=01h=Folder:                                            |
|                                                                       |
|   004h 4    Folder Parent Index (0=Root, 1=Dot, Other=NestedSubdir's) |
|       008h                                                            |
|  4    Folder End Index (entries up to excluding End Index are childs) |
+-----------------------------------------------------------------------+
```


**Special Cases**
3DS Face Raiders RomFS:\\hal\\effect\\kbb\\kbb.dat doesn\'t have a \".\
dot folder.
3DS Mii Maker RomFS:\\shaders\\shader.arc has nested folders & empt
folders.
