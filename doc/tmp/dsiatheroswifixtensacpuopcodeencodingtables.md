...
**Xtensa Opcodes**
Xtensa opcodes are 24bit wide (or 16bit for \"narrow\" opcodes), th
opcodes consist of several 4bit fields (or 2bit, 8bit, 12bit, 16bit,
18bit fields in some cases):
...
Opcode decoding can be done in 4bit units, starting at the \"op0\
field, and then decoding further 4bit field(s) depending on the opcode.
Decoding speed could be improved by grouping two 4bit fields into a 8bit
field (eg. op2 and op1; that won\'t work out perfectly for all opcode
though).
...
|       ROOT\CALLN                   op0=5, mn                          
|       RO                                                              |
| OT\SI                      op0=6, mn   (and \SI\BZ, \SI\BI0, \SI\BI1) 
...
Below is showing the whole opcode \"tree\", starting with the root tabl
(indexed via op0). Entries with \"\--\>\" are referencing to chil
tables, the other entries are indicating the actual opcodes (or reserved
opcodes). The lower-case suffices are somewhat indicating optional
opcodes.

```
+-----------------------------------------------------------------------+
|         ROO                                                           |
| T\             ROOT\QRST         ROOT\QRST\RS      ROOT\QRST\RST0\ST0 
|         op0               op1               op2               r       |
|                                                                       |
...
+-----------------------------------------------------------------------+
|         ROOT                                                          |
| \QRST\LSCX    ROOT\QRST\LSC4    ROOT\QRST\FP0     ROOT\QRST\FP0\FP1OP 
|         op2               op2               op2               t       |
...
+-----------------------------------------------------------------------+
|         R                                                             |
| OOT\MAC16\MACID  ROOT\MAC16\MACCD  ROOT\MAC16\MACIA  ROOT\MAC16\MACCA 
|         op1               op1               op1               op1     |
|                                                                       |
...
|      7 reserved          reserved          reserved          reserved |
|       8 M                                                             |
| ULA.DD.LL.LDINC  MULA.DD.LL.LDDEC  MULA.DA.LL.LDINC  MULA.DA.LL.LDDEC |
|       9 M                                                             |
| ULA.DD.HL.LDINC  MULA.DD.HL.LDDEC  MULA.DA.HL.LDINC  MULA.DA.HL.LDDEC |
|       A M                                                             |
| ULA.DD.LH.LDINC  MULA.DD.LH.LDDEC  MULA.DA.LH.LDINC  MULA.DA.LH.LDDEC |
|       B M                                                             |
| ULA.DD.HH.LDINC  MULA.DD.HH.LDDEC  MULA.DA.HH.LDINC  MULA.DA.HH.LDDEC |
|                                                                       |
|      C reserved          reserved          reserved          reserved |
|                                                                       |
|      D reserved          reserved          reserved          reserved |
...
+-----------------------------------------------------------------------+
|         R                                                             |
| OOT\MAC16\MACDD  ROOT\MAC16\MACAD  ROOT\MAC16\MACDA  ROOT\MAC16\MACAA 
|         op1               op1               op1               op1     |
|                                                                       |
