...
**Gameshark RAW Codes (These codes must be encrypted before using
them)**
...
|       3000ccc                                                         |
| c xxxxxxxx  write xxxxxxxx to (cccc-1) addresses (list in next codes) |
|       aaaaa                                                           |
| aaa aaaaaaaa  parameter for above code, containing two addresses each |
|       aaaaaaa                                                         |
| a 00000000  last parameter for above, zero-padded if only one address |
...
|       80F000                                                          |
| 00 0000xxxx  IF GS_Button_Down THEN slowdown xxxx * ? cycles per hook |
...
|       xxxxxxx                                                         |
- | x 001DC0DE  Enable Code - Game Code ID (value at [0ACh] in cartridge) |
...
Note: V1/V2 hardware can only have up to 1 user-defined rom patch max.
V3 can have up to 4. Some enable code types can shorten the amount of
user-defined rom patches available.
