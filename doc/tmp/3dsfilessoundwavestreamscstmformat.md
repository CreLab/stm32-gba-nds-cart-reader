...
CSTM files are very similar to CWAV files, with slightly differently
arranged data, and with an additional SEEK block.
Unknown if/which games or system titles are using CSTM files. They can
be reportedly found in following locations:

```
+-----------------------------------------------------------------------+
|       - RomFS:\sound\stream\*.bcstm (eg. in Zelda Ocarina of Time)    
|       - I                                                             |
| nside of a CSAR (unknown where exactly, maybe inside of CWAR blocks?) |
|       - CSAR                                                          |
| 's can contain RefID=220Dh entries for external CSTM file (eg. Zelda) |
+-----------------------------------------------------------------------+
```

CSTM seems to be intended for streaming medium length recordings (longer
than normal CWAV instruments/effects, and shorter than fully fledged
AAC/OGG/MP3 recordings).
...
|       012h 2      Reserved                                            |
|       014                                                             |
| h 4+4+4  INFO Block Reference (RefID=4000h, Offset from CSTM+0, Size) |
|       020                                                             |
| h 4+4+4  SEEK Block Reference (RefID=4001h, Offset from CSTM+0, Size) |
|       02C                                                             |
| h 4+4+4  DATA Block Reference (RefID=4002h, Offset from CSTM+0, Size) |
...
The info block is aligned to 20h bytes. uh, that is, the \"Stream Info\
block?
...
|       030h                                                            |
| 4+4  Sample Data   (RefID=1F00h, Offset from "Data Block Data field") |
+-----------------------------------------------------------------------+
```
...
|       002h 2    Padding                                               |
|       004h                                                            |
| 4+4  Channel Index Byte Table (RefID=0100h, Offset from Volume field) |
...
+-----------------------------------------------------------------------+
|       00                                                              |
| 0h 4+4  DSP-ADPCM Info  (RefID=0300h, Offset from here?)  ;\depending 
|                                                                       |
...
|       00                                                              |
| 2h 1   Table Index                   ; ...for port 10103418h+(N*20h)? |
|       003h 1   Padding                       ;/                       |
|       004h 2   Data                          ;\Loop context           
|       00                                                              |
| 6h 1   Table Index                   ; ...for port 1010341Ch+(N*20h)? |
|       007h 1   Padding                       ;/                       |
+-----------------------------------------------------------------------+
```
...
+-----------------------------------------------------------------------+
|      ________                                                         |
| ________________________ SEEK Block _________________________________ |
+-----------------------------------------------------------------------+
```
...
+-----------------------------------------------------------------------+
|      ________                                                         |
| ________________________ DATA Block _________________________________ |
+-----------------------------------------------------------------------+
```
...
The data block is aligned to 20h bytes, as well as the data field\'
actual sample data.
...
+-----------------------------------------------------------------------+
|      ________                                                         |
| ________________________ Notes..... _________________________________ |
+-----------------------------------------------------------------------+
```
