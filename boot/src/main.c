...
inline static void gpio_set_mode(uint32_t gpiodev, uint16_t gpion, uint8_t mode) {
	if (gpion < 8)
		GPIO_CRL(gpiodev) = (GPIO_CRL(gpiodev) & ~((uint32_t)0xf << ((gpion)<<2))) | (mode << ((gpion)<<2));
	else
		GPIO_CRH(gpiodev) = (GPIO_CRH(gpiodev) & ~((uint32_t)0xf << ((gpion-8)<<2))) | (mode << ((gpion-8)<<2));
}
...
#define RCC_CFGR_SW_SHIFT                 (uint32_t)0
#define RCC_CFGR_SW (uint32_t)(3 << RCC_CFGR_SW_SHIFT)
...
static void clock_setup_in_hse_12mhz_out_72mhz() {
    /* Enable external high-speed oscillator 12MHz. */
    RCC_CR |= RCC_CR_HSEON;
    while (!(RCC_CR & RCC_CR_HSERDY));

    /*
     * Prescalers: AHB = /1, APB1 = /2, APB2 = /1, ADC = /8
     * PLL source = HSE, PLL mul = x6 (12MHz * 6 = 72MHz)
     */
    uint32_t reg32 = RCC_CFGR & 0xFFC0000F;
    reg32 |= (RCC_CFGR_HPRE_SYSCLK_NODIV << 4) |
             (RCC_CFGR_PPRE1_HCLK_DIV2 << 8) |
             (RCC_CFGR_PPRE2_HCLK_NODIV << 11) |
             (RCC_CFGR_ADCPRE_PCLK2_DIV8 << 14) |
             (RCC_CFGR_PLLSRC_HSE_CLK << 16) |
             (RCC_CFGR_PLLXTPRE_HSE_CLK << 17) |
             (RCC_CFGR_PLLMUL_PLL_CLK_MUL6 << 18);   // <-- WICHTIG: x6 statt x9
    RCC_CFGR = reg32;

    /* Flash wait states: 2WS for 72MHz */
...
int main(void) {
	...
	if (!rebooted_into_updater() && (FLASH_WRPR & 1)) {
		...
		opt[WORD_WRP0] &= (uint16_t)~0x0001;    // Bit 0 write protects pages 0-3 (4KB)
